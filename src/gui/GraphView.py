...
class GraphView(tk.Frame):
    def __init__(self, parent, func, **kwargs):
        ...
        # Dynamic label that details the x-value converted into radians, which is the value used for the graph plot.
        lbl_xvalue_radians = tk.Label(self, text="X Value in Radians:", font="Arial 12 bold", bg="white")
        lbl_xvalue_radians.grid(row=6, column=0, columnspan=2, padx=(25, 5), sticky=tk.E)

        dyn_xvalue_radians = tk.Label(self, font="Arial 12 bold", bg="white")
        dyn_xvalue_radians.grid(row=6, column=2, columnspan=2, padx=(5, 25), sticky=tk.W)

        # Dynamic label that details the tangent line equation.
        lbl_tangent_eq = tk.Label(self, text="Tangent Line Equation:", font="Arial 12 bold", bg="white")
        lbl_tangent_eq.grid(row=7, column=0, columnspan=2, padx=(25, 5), pady=(0, 5), sticky=tk.E)

        dyn_tangent_eq = tk.Label(self, font="Arial 12 bold", bg="white")
        dyn_tangent_eq.grid(row=7, column=2, columnspan=2, padx=(5, 25), pady=(0, 5), sticky=tk.W)

        # Dynamic label that details the normal line equation.
        lbl_normal_eq = tk.Label(self, text="Normal Line Equation:", font="Arial 12 bold", bg="white")
        lbl_normal_eq.grid(row=8, column=0, columnspan=2, padx=(25, 5), pady=(0, 5), sticky=tk.E)

        dyn_normal_eq = tk.Label(self, font="Arial 12 bold", bg="white")
        dyn_normal_eq.grid(row=8, column=2, columnspan=2, padx=(5, 25), pady=(0, 5), sticky=tk.W)

        # Dynamic label that details the scaled normal line equation that is actually displayed.
        lbl_scaled_eq = tk.Label(self, text="Scaled Normal Line Equation:", font="Arial 12 bold", bg="white")
        lbl_scaled_eq.grid(row=9, column=0, columnspan=2, padx=(25, 5), pady=(0, 5), sticky=tk.E)

        dyn_scaled_eq = tk.Label(self, font="Arial 12 bold", bg="white")
        dyn_scaled_eq.grid(row=9, column=2, columnspan=2, padx=(5, 25), pady=(0, 5), sticky=tk.W)
        ...
        lbl_amplitude.grid(row=12, column=4, padx=5, pady=(0, 5), sticky=tk.N)
        ...
        lbl_frequency.grid(row=12, column=5, padx=5, pady=(0, 5), sticky=tk.N)
    ...
    def apply_changes(self, graph, dyn, x_dyn, tan_dyn, nom_dyn, scl_dyn, tan_chk, nom_chk, x, amp, freq, phase):
        ...
        if (x != ""):
            x_dyn.config(text=self.calc.update_xvalue_label(self.calc.convert_to_float(x)))
